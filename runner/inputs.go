...
func (process *Process) bindInput(
	...
	case "array":
		...
		out := []*Binding{}
		t := types.MustArraySchema()
		for i, val := range vals {
			subkey := append(key, sortKey{getPos(types.Binding), i}...)
			b, err := process.bindInput("",
				cwl.CommandInputType{ SaladType: t.Items }, types.Binding, nil, val, subkey)
			if err != nil {
				return nil, err
			}
			if b == nil {
		...
		copy(nested, out)
		b := &Binding{clb, types, val, key, nested, name}
