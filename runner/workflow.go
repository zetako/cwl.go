...
func (engine *K8sEngine) runDocument() error {
	...
	if root.Class == "CommandLineTool" {
		...
		if err = engine.run(mainTask); err != nil {
			return engine.errorf("failed to run tool task: %v", err)
		}
		return nil
	}
	...
	if root.Steps.Len() != 0 && root.Graphs.Len() == 0 {
		// Load Graphs From Importer
		for _ ,stepi := range root.Steps {
			if stepi.Run.Workflow == nil && stepi.Run.Value != "" {
				stepRoot , err := engine.loadRoot(stepi.Run.Value)
				if err !=nil {
					return engine.errorf("failed to load graph : %v", err)
				}
				stepRoot.ID = stepi.Run.Value
				root.Graphs = append(root.Graphs, stepRoot)
			}
		}
		root.ID = mainProcessID
		root.Graphs = append(root.Graphs, &root)
