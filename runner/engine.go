...
type TaskRuntimeJSContext struct {
	...
	Cores int `json:"cores"` // number of CPU cores reserved for the tool process
	RAM int `json:"ram"`	// amount of RAM in mebibytes (2**20) reserved for the tool process
	OutdirSize string `json:"outdirSize"` // reserved storage space available in the designated output directory
	TmpdirSize string `json:"tmpdirSize"` // reserved storage space available in the designated output directory
}
...
func Engine(runID string,raw []byte) (err error) {
	engine := engine(runID)
	...
	if err = engine.loadRequest(raw); err != nil {
		return engine.errorf("failed to load workflow request: %v", err)
	}
	if err = engine.runDocument(); err != nil {
...
func (engine *K8sEngine) loadRequest(raw []byte) error {
...
func (tool *Tool) newJSVM() *otto.Otto {
	vm := otto.New()
	runtime := &TaskRuntimeJSContext{
		Outdir: tool.WorkingDir,
		Cores: 1, // TODO
		RAM: 1024, // TODO
		Tmpdir: "/tmp", // TODO
	}
...
func (engine *K8sEngine) setupTool(tool *Tool) (err error) {
	...
	// loads inputs context to js vm tool.InputsVM (NOTE: Ready to test, but needs to be extended)
	if err = tool.runtimeToVM(); err != nil {
		return tool.Task.errorf("failed to load inputs to js vm: %v", err)
	}
	...
	engine.infof("TODO writeFileInput")
